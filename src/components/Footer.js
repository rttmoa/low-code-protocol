...
/**--- Footer组件 ---**/
export function Footer({ children, previous, next }) { // <Link><a></a></Link>跳转，上一页，下一页
    ...
    <footer className={clsx('text-sm leading-6', previous || next ? 'mt-12' : 'mt-16')}>
      ...
      <div className="pt-10 pb-28 border-t border-slate-200 sm:flex justify-between text-slate-500 dark:border-slate-200/5">
        <div className="mb-6 sm:mb-0 sm:flex">
          <p>&copy; {new Date().getFullYear()} Low Code Protocol</p>
        ...
        {/* 有children渲染children，没有children渲染底部盒子  -  !children */}
        {children ? (
          children
        ) : (
          <div className="flex space-x-10 text-slate-400 dark:text-slate-500">
            <a href="https://github.com/steedos/low-code-protocol" className="hover:text-slate-500 dark:hover:text-slate-400" >
