...
import { Dialog } from '@headlessui/react'   //-->    headlessui UI库
...
/**--- NavItem每一个子导航项，  forwordRef：引用传递是一种通过组件向子组件自动传递引用ref的技术 ---**/
// 每一个小li是侧边栏的二级导航
// <a>{children}</a>  这个children是文字内容
...
/**
 * 找到最近的可滚动祖先（如果可滚动则为 self）
 * 从 smoothscroll polyfill 改编和简化的代码
 * @param {Element} el
 */
...
function Nav({ nav, children, fallbackHref, mobile = false }) {
  ...
  // useIsomorphicLayoutEffect = typeof window !== 'undefined' ? useLayoutEffect : useEffect
      ...
      <ul>
        ...
        {nav &&
            Object.keys(nav).map(category => {
              // console.log(category) // 可视化工具
              // console.log(nav[category]) // [{title: '对象编辑器', href: '/docs/designer-object'},.....]
